  ...
  <header className={`sticky top-0 z-10 w-full ${cardBg} border-b border-gray-700 shadow-xl`}>
    <div className="max-w-7xl mx-auto flex justify-between items-center h-16 px-4 sm:px-6 lg:px-8">
      ...
      <nav className="hidden md:flex space-x-6">
        ...
        <a href="/leaderboard" className={`text-sm font-medium ${primaryTextColor} hover:${neonAccent} transition duration-150`}>Leaderboard</a>
      </nav>
    </div>
  </header>
);
...
export default function QuarterPongRulesPage() {
  return (
    // Outer container for the full website layout (min-height screen, vertical flex)
    <div className={`min-h-screen flex flex-col ${primaryBg}`}>
      ...
      <main className={`flex-grow py-12 px-4 sm:px-6 lg:px-8`}>
        {/* Increased max-width for better website layout on desktop (max-w-6xl) */}
        <div className="max-w-6xl mx-auto space-y-12">
          
          {/* Rules Title Card (Big, centered banner) */}
          <header className={`text-center p-8 rounded-xl ${cardBg} shadow-2xl border-b-4 border-cyan-400/50`}>
            <h1 className={`text-6xl font-extrabold tracking-widest uppercase ${neonAccent} drop-shadow-xl`}>
              Official Rules & Game Modes
            </h1>
            <p className="text-xl mt-3 text-gray-400">Everything you need to master the game.</p>
          </header>
